...
describe('submit flow (integration-style)', () => {
  it('should signal that override is required when splintering detected', async () => {
    ...
    const result = await splinter.checkSplintering({} as any, { requesterId: request.requesterId, departmentId: request.departmentId, total: request.totalEstimated, windowDays: 30, threshold: 250000 });
