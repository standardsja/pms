...
> {
    const { thresholdTitle = 0.7, thresholdDescription = 0.6, limit = 5, excludeRejected = true } = options;
    ...
    try {
        ...
        const duplicates = existingIdeas
            ...
            // Filter by threshold
            .filter((item) => item.titleSimilarity >= thresholdTitle || item.descriptionSimilarity >= thresholdDescription || item.overallSimilarity >= Math.min(thresholdTitle, thresholdDescription))
 ...
 */
export async function isDuplicateIdea(title: string, description: string): Promise<boolean> {
