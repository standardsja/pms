...
export async function batchUpdateIdeas(
  ...
  const failed: number[] = [];
  const status: IdeaStatus = action === 'APPROVE' ? IdeaStatus.APPROVED : IdeaStatus.REJECTED;
...
export async function getCommitteeDashboardStats(): Promise<{
  ...
  try {
    ...
    const [
      ...
      prisma.idea.count({ where: { status: IdeaStatus.PENDING_REVIEW } }),
      prisma.idea.count({ where: { status: IdeaStatus.APPROVED } }),
      prisma.idea.count({ where: { status: IdeaStatus.REJECTED } }),
      prisma.idea.count({ where: { status: IdeaStatus.PROMOTED_TO_PROJECT } }),
      prisma.idea.count({
        where: {
      ...
      prisma.idea.findMany({
        where: {
          reviewedAt: { not: null },
          status: { in: [IdeaStatus.APPROVED, IdeaStatus.REJECTED] },
        },
        select: {
          submittedAt: true,
          reviewedAt: true,
 ...
 */
export async function getPendingIdeasForReview(options: {
  limit?: number;
  offset?: number;
  sortBy?: 'recent' | 'votes' | 'oldest';
  category?: string;
}): Promise<{
  ideas: Array<{
    id: number;
...
export async function getCommitteeMemberStats(userId: number): Promise<{
  ...
  try {
    ...
    // Calculate approval rate
    const approved = reviewedIdeas.filter(i => i.status === IdeaStatus.APPROVED).length;
