 ...
 */
export function checkProcurementThresholds(
    totalValue: number,
    ...
    const isGoodsOrServices = types.some(type => 
        ['goods', 'services', 'consulting', 'supplies', 'equipment', 'materials'].includes(type)
    );
...
): Promise<{ nextStatus: string; nextAssigneeId: number | null; comment: string }> {
 ...
 */
export async function logThresholdDecision(
    requestId: number,
    userId: number,
    thresholdResult: ThresholdResult,
    totalValue: number,
