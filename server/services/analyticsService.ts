...
export async function calculateAnalytics(): Promise<AnalyticsCache> {
  ...
  try {
    ...
    const submissionTrendsRaw = calculateDailyTrends(recentIdeas.map((i: any) => i.createdAt), 30);
    ...
    const votingTrendsRaw = calculateDailyTrends(recentVotes.map((v: any) => v.createdAt), 30);
