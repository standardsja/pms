 ...
 */
export function calculateTrendingScore(upvotes: number, downvotes: number, views: number, createdAt: Date): number {
    const now = new Date();
...
export function initTrendingScoreJob(): NodeJS.Timeout {
    ...
    updateAllTrendingScores().catch((err) => console.error('[Trending] Initial update failed:', err));

    // Then run every hour
    const interval = setInterval(() => {
        updateAllTrendingScores().catch((err) => console.error('[Trending] Scheduled update failed:', err));
    }, 60 * 60 * 1000); // 1 hour
