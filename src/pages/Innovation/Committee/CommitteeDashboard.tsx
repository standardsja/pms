...
const CommitteeDashboard = () => {
    ...
    const availableCategories = useMemo(() => {
        ...
        (selectedTab === 'pending' ? pendingIdeas : selectedTab === 'approved' ? approvedIdeas : promotedIdeas)
    ...
    const exportCsv = () => {
        ...
        filteredIdeas.forEach(i => rows.push([
            i.id, i.title, i.category, i.submittedBy, i.submittedAt, 
            i.status, String(i.voteCount), String(i.viewCount)
        ]));
            ...
            <div className="relative overflow-hidden rounded-xl bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 p-8 text-white shadow-xl">
                <div className="relative z-10">
                    ...
                    <p className="text-indigo-100 text-lg">
                        Review community ideas, manage approvals, and guide BSJ innovation initiatives
                    </p>
                </div>
                            ...
                            {paginatedIdeas.map((idea) => (
                                <div key={idea.id} className="border border-gray-200 dark:border-gray-700 rounded-lg p-6 hover:shadow-lg transition-shadow bg-white dark:bg-gray-800">
                                    <div className="flex items-start justify-between gap-4 mb-4">
                                        <div className="flex-1">
                                            <div className="flex items-start gap-3 mb-3">
                                                <span className={`px-3 py-1 rounded-full text-xs font-bold ${getCategoryColor(idea.category)}`}>
                                                    {getCategoryLabel(idea.category)}
                                                </span>
                                                {selectedTab === 'promoted' && idea.projectCode && (
                                                    <span className="px-3 py-1 rounded-full text-xs font-bold bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-300">
                                                        {idea.projectCode}
                                                    </span>
                                                )}
                                            </div>
                                            ...
                                            <div className="flex items-center gap-4 text-sm">
                                                ...
                                                <span className="text-gray-600 dark:text-gray-400">
                                                    Score: {idea.voteCount > 0 ? `+${idea.voteCount}` : idea.voteCount}
                                                </span>
                                                <span>•</span>
                                    ...
                                    <div className="flex items-center gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
                                        ...
                                        {selectedTab === 'promoted' && idea.projectCode && (
                                            <button
                                                onClick={() => copyProjectCode(idea.projectCode!)}
                                        ...
                                        <button
                                            onClick={() => shareIdea(idea.id, idea.title)}
                                            className="btn btn-outline-secondary"
            ...
            {showPromoteModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                    <div className="panel w-full max-w-lg">
                        ...
                        <p className="text-sm text-gray-600 dark:text-gray-400 mb-3">
                            Enter a project code (optional - will auto-generate if blank)
                        </p>
                        <input 
                            type="text" 
