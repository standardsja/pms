...
export default function ReviewIdeas() {
    ...
    const submitBulkNotes = async () => {
        ...
        try {
            const promises = selectedList.map((idea) => (bulkAction === 'approve' ? approveIdea(idea.id, bulkNotesText || undefined) : rejectIdea(idea.id, bulkNotesText || undefined)));
        ...
        <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        ...
                        <div>
                            ...
                            <select
                                ...
                                onChange={(e) => {
                                    setSubmitterFilter(e.target.value === 'all' ? '' : e.target.value);
                                    setPage(1);
                                }}
            ...
            <div className="panel">
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
                    ...
                    {active === 'pending' && (
                        <div className="flex items-center gap-2">
                            <button onClick={selectAll} className="btn btn-outline-secondary btn-sm" disabled={filtered.length === 0}>
                ...
                <div className="mt-2 text-xs text-gray-500 dark:text-gray-400 flex flex-wrap gap-3">
                    <span>
                        <kbd className="px-1.5 py-0.5 bg-gray-100 dark:bg-gray-800 rounded">/</kbd> Focus search
                    </span>
                    <span>
                    ...
                    {active === 'pending' && (
                        <>
                            <span>
                                <kbd className="px-1.5 py-0.5 bg-gray-100 dark:bg-gray-800 rounded">A</kbd> Approve
                            </span>
                            <span>
                                <kbd className="px-1.5 py-0.5 bg-gray-100 dark:bg-gray-800 rounded">R</kbd> Reject
                            </span>
                        </>
                    )}
                    {(active === 'approved' || active === 'popular') && (
                        <span>
                            <kbd className="px-1.5 py-0.5 bg-gray-100 dark:bg-gray-800 rounded">P</kbd> Promote
                        </span>
                    )}
                </div>
                ...
                {!loading && !error && filtered.length === 0 && <div className="py-12 text-center text-gray-500">{emptyMessage}</div>}
                    ...
                    <>
                        <div ref={ideasListRef} className="divide-y divide-gray-200 dark:divide-gray-700">
                            {paginatedIdeas.map((idea, idx) => (
                                <div
                                    key={idea.id}
                                    className={`py-4 flex flex-col md:flex-row md:items-center md:justify-between gap-4 ${focusedIndex === idx ? 'bg-blue-50 dark:bg-blue-900/20 -mx-4 px-4' : ''}`}
                                >
                                    <div className="flex items-start gap-3 flex-1">
                                        {/* Checkbox for bulk selection (pending only) */}
                                        {active === 'pending' && <input type="checkbox" checked={selectedIds.has(idea.id)} onChange={() => toggleSelection(idea.id)} className="form-checkbox mt-1" />}
                                    ...
                                    <div className="flex items-center gap-2">
                                        {active === 'pending' && (
                                            <>
                                                <button className="btn btn-success" onClick={() => onApprove(idea)}>
                                                    Approve
                                                </button>
                                                <button className="btn btn-danger" onClick={() => onReject(idea)}>
                                                    Reject
                                                </button>
                                            </>
                                        )}
                        ...
                        {totalPages > 1 && (
                            <div className="mt-4 flex items-center justify-between">
                                ...
                                <div className="flex items-center gap-2">
                                    <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1} className="btn btn-outline-primary btn-sm">
                                        Previous
                                    </button>
                                    <span className="text-sm text-gray-600 dark:text-gray-400">
                                        Page {page} of {totalPages}
                                    </span>
                                    <button onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page === totalPages} className="btn btn-outline-primary btn-sm">
                                        Next
                                    </button>
                                </div>
                            </div>
            ...
            {showBulkNotes && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                    <div className="panel w-full max-w-lg">
                        ...
                        <p className="text-sm text-gray-600 dark:text-gray-400 mb-3">Add shared review notes for all selected ideas (optional).</p>
                        <textarea className="form-textarea w-full mb-4" rows={4} value={bulkNotesText} onChange={(e) => setBulkNotesText(e.target.value)} placeholder="Add notes (optional)" />
                        <div className="flex justify-end gap-2">
                            <button
                                className="btn btn-outline-secondary"
                                onClick={() => {
                            ...
                            <button className={`btn ${bulkAction === 'approve' ? 'btn-success' : 'btn-danger'}`} onClick={submitBulkNotes}>
