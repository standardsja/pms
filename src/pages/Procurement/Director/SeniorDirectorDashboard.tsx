...
const SeniorDirectorDashboard = () => {
    ...
    useEffect(() => {
        dispatch(setPageTitle('Senior Director Dashboard'));
        loadData();
    }, [dispatch]);
    ...
    const loadData = async () => {
        ...
        try {
            ...
            const criticalCount = data.requests?.filter((req: Request) => req.priority === 'HIGH').length || 0;

            setStats({
                pendingApprovals: pendingCount,
                totalApprovedValue: totalValue,
                monthlySpend: totalValue,
    ...
    return (
        <div>
            <div className="mb-6">
                <h1 className="text-2xl font-bold">Senior Director Dashboard</h1>
                <p className="text-gray-500 dark:text-gray-400">Final approval authority for all procurement requests</p>
            </div>
            ...
            <div className="mb-6 grid gap-6 sm:grid-cols-2 xl:grid-cols-4">
                <div className="panel h-full">
                    <div className="flex items-center justify-between">
                        <div className="text-md font-semibold ltr:mr-1 rtl:ml-1">Pending Final Approvals</div>
                        <IconClock className="h-5 w-5 text-warning" />
                    </div>
                    <div className="mt-5 flex items-center">
                        <div className="text-3xl font-bold ltr:mr-3 rtl:ml-3">{stats.pendingApprovals}</div>
                    </div>
                </div>

                <div className="panel h-full">
                    <div className="flex items-center justify-between">
                        <div className="text-md font-semibold ltr:mr-1 rtl:ml-1">Total Pending Value</div>
                ...
                <div className="panel h-full">
                    <div className="flex items-center justify-between">
                        <div className="text-md font-semibold ltr:mr-1 rtl:ml-1">Monthly Spend</div>
                        <IconTrendingUp className="h-5 w-5 text-success" />
                    </div>
                    <div className="mt-5 flex items-center">
                        <div className="text-3xl font-bold ltr:mr-3 rtl:ml-3">{formatCurrency(stats.monthlySpend)}</div>
                    </div>
                </div>

                <div className="panel h-full">
                    <div className="flex items-center justify-between">
                        <div className="text-md font-semibold ltr:mr-1 rtl:ml-1">Critical Priority Requests</div>
                        <IconClock className="h-5 w-5 text-danger" />
                    </div>
                    <div className="mt-5 flex items-center">
                        <div className="text-3xl font-bold ltr:mr-3 rtl:ml-3">{stats.criticalRequests}</div>
                    </div>
                </div>
            </div>
            ...
            <div className="panel">
                <div className="mb-5 flex items-center justify-between">
                    <h5 className="text-lg font-semibold dark:text-white-light">Requests Requiring Final Approval</h5>
                </div>
...
export default SeniorDirectorDashboard;
