                ...
                <thead>
                    <tr>
                        {columns.map((col) => (
                            <th key={col.id} className="border px-2 py-1">
                                ...
                                {structureEditable && onRemoveColumn && (
                                    <button type="button" className="ml-2 text-red-500" onClick={() => onRemoveColumn(col.id)} title="Remove column">
                                        ×
                                    </button>
                                )}
                            </th>
                        ...
                        {structureEditable && onAddColumn && (
                            <th>
                                <button type="button" className="btn btn-xs" onClick={onAddColumn}>
                                    + Col
                                </button>
                            </th>
                        )}
                        ...
                        <tr key={row.id}>
                            {columns.map((col) => (
                                <td key={col.id} className="border px-2 py-1">
                                    {editable ? (
                                        <input className="form-input w-full" value={row.data[col.id] || ''} onChange={(e) => onCellChange(row.id, col.id, e.target.value)} disabled={!editable} />
                                    ) : (
                                        row.data[col.id] || ''
                                    )}
                                </td>
                            ...
                            {structureEditable && onRemoveRow && (
                                <td>
                                    <button type="button" className="btn btn-xs text-red-500" onClick={() => onRemoveRow(row.id)}>
                                        × Row
                                    </button>
                                </td>
                            )}
                    ...
                    {structureEditable && onAddRow && (
                        <tr>
                            <td colSpan={columns.length + 1}>
                                <button type="button" className="btn btn-xs" onClick={onAddRow}>
                                    + Row
                                </button>
                            </td>
                        </tr>
