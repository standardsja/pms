    ...
    <div className="panel">
      ...
      <div className="mb-4">
        ...
        <div className="mt-2 flex gap-2">
          <button className="btn btn-primary" onClick={() => handlePost(null)} disabled={posting || !text.trim()}>Post</button>
          {replyTo && <button className="btn btn-outline" onClick={() => setReplyTo(null)}>Cancel reply</button>}
        </div>
      </div>
      ...
      <div className="space-y-4">
        {tree.map((c) => (
          <CommentNode key={c.id} node={c} meId={me?.id ? Number(me.id) : undefined} onReply={(id) => setReplyTo(id)} onDelete={handleDelete} onSubmitReply={(txt, pid) => { setText(txt); setReplyTo(pid); }} />
        ))}
      </div>
    </div>
  );
...
function CommentNode({ node, meId, onReply, onDelete, onSubmitReply }: {
    ...
    <div className="border border-gray-200 dark:border-gray-700 rounded p-3">
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex items-center gap-2 text-xs">
          <button className="text-primary hover:underline" onClick={() => { onReply(node.id); setShowReply(true); }}>Reply</button>
          {meId === node.userId && (
            <button className="text-red-600 hover:underline" onClick={() => onDelete(node.id)}>Delete</button>
          )}
        </div>
      </div>
      ...
      {showReply && (
        <div className="mt-2">
          ...
          />
          <div className="mt-2 flex gap-2">
            <button className="btn btn-primary" onClick={submit}>Post reply</button>
            <button className="btn btn-outline" onClick={() => setShowReply(false)}>Cancel</button>
          </div>
        </div>
      )}
